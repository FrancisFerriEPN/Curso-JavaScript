...
//<<<
    ...
    const resultado = obtener_autos().filter(filtrar_marca).filter(filtrar_year).filter(filtrar_minimo).filter(filtrar_maximo).filter(filtrar_puertas).filter(filtrar_transmision).filter(filtrar_color);
    if (resultado.length){
